...
TASKS = {
    "Task01_BrainTumour": {"name": "brain", "classes": ["edema", "ne_tumour", "e_tumour"],
            ...
            for j in range(n_slices):
                ...
                for k, subtask in enumerate(task_metadata["classes"]):
                    directory_name = (task_metadata["name"] +
                                      ...
                                      ("_" + mode if mode != "" else ""))
                    nothresh_dir = directory_name + "_nothresh"
                    ...
                    nothresh_image_target_dir = root_dir / nothresh_dir / "images"
                    nothresh_mask_target_dir = root_dir / nothresh_dir / "masks"
                    os.makedirs(nothresh_image_target_dir, exist_ok=True)
                    os.makedirs(nothresh_mask_target_dir, exist_ok=True)
                    ...
                    if add_slice:
                        ...
                        image_target = nothresh_image_target_dir / (base_name + "_slice" + str(j) + ".{}".format(output_format))
                        mask_target = nothresh_mask_target_dir / (base_name + "_slice" + str(j) + ".{}".format(output_format))
                        # print("Slice should be added")
                        k_mask = mask_slice.copy()
                        # zero everything that's not region of interest
                        k_mask[k_mask != (k + 1)] = 0
...
def process_all_tasks(root, tasks_list = TASKS.keys(), norm_ct=False,
                      normalize_volume=False,
                      output_format="png"):
    for key in tasks_list:
        data_root = Path("/project/ajiteshs_1061/project7/Data")
        path = data_root / key
        ...
        images_file_paths = glob.glob(f"{images_path}/*.nii.gz")
        # masks_file_paths = masks_path.glob("**/*.nii.gz")
        len_images = len(list(images_file_paths))
        for path in tqdm.tqdm(list(images_file_paths), total=len_images):
            path = Path(path)
            if not (path.name).startswith("._"):
                # fie name - must be the same for masks and image
