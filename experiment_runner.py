...
from meta_learning.amphibian import Amphibian
...
def meta_train(root_dir, params, exclude, target_name="meta_weights.pth"):
    ...
    amphibian = Amphibian(meta_dataset, hyperparams=params,
                      verbosity_level=2, seed=6, pso_config=None,
                      reference_set=reference_loader, volumetric=volumetric)
    start = timer()
    ...
    meta_results = amphibian.get_meta_results()
    results = {"ex": ex_name,
               "weights": save_path,
               "elapsed": elapsed
               }
...
if __name__ == '__main__':
    ...
    data_dir = Path("/project/ajiteshs_1061/project7/BackupCode/data/organs")
    exp_type = args.exp_type
    params_file = args.params_file
    with open(params_file) as json_file:
        data = json.load(json_file)

    if data.get("params").get("chart_name"):
	    wandb.init(
		project="EE638",
		name=data.get("params").get("chart_name"),
