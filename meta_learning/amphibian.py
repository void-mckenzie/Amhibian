...
from networks.attunet import UNet
...
DATASET_WEIGHTS = {
    'brain_edema_FLAIR': 1/12,
    'brain_edema_t1gd': 1/12,
    'brain_edema_T1w': 1/12,
    'brain_edema_T2w': 1/12,
    'brain_e_tumour_FLAIR': 1/12,
    'brain_e_tumour_t1gd': 1/12,
    'brain_e_tumour_T1w': 1/12,
    'brain_e_tumour_T2w': 1/12,
    'brain_ne_tumour_FLAIR': 1/12,
    'brain_ne_tumour_t1gd': 1/12,
    'brain_ne_tumour_T1w': 1/12,
    'brain_ne_tumour_T2w': 1/12,
    'colon': 1,
    'heart': 1,
    'hippocampus_anterior': 1/2,
    'hippocampus_posterior': 1/2,
    'lung': 1,
    'pancreas': 1/2,
    'pancreas_cancer': 1/2,
    'prostate_peripheral_T2': 1/2,
    'prostate_transitional_T2': 1/2,
    'spleen': 1,
    'vessel': 1/2,
    'vessel_cancer': 1/2}
...
DATASET_COUNTS = {
    ...
    'brain_edema_T1w': 0,
    'brain_edema_T2w': 0,
    'brain_e_tumour_FLAIR': 0,
    'brain_e_tumour_t1gd': 0,
    'brain_e_tumour_T1w': 0,
    'brain_e_tumour_T2w': 0,
    'brain_ne_tumour_FLAIR': 0,
    'brain_ne_tumour_t1gd': 0,
    'brain_ne_tumour_T1w': 0,
    'brain_ne_tumour_T2w': 0,
    'colon': 0,
    'heart': 0,
    'hippocampus_anterior': 0,
    'hippocampus_posterior': 0,
    'lung': 0,
    'pancreas': 0,
    'pancreas_cancer': 0,
    'prostate_peripheral_T2': 0,
    'prostate_transitional_T2': 0,
    'spleen': 0,
    'vessel': 0,
    'vessel_cancer': 0
    }
...
class Amphibian:
    def __init__(self, meta_dataset, hyperparams=None,
        ...
        if self.model_name == "UNet":
                self.model = UNet_Cells(n_class=1, affine=False, sigmoid=self.sigmoid, norm_type=self.norm_type)
        elif self.model_name == "AttUNet5":
                self.model = UNet(1,1,5 )
        elif self.model_name == "AttUNet2":
                self.model = UNet(1,1,2 )
        elif self.model_name == "AttUNet3":
                self.model = UNet(1,1,3 )
        elif self.model_name == "AttUNet7":
                self.model = UNet(1,1,7 )
        elif self.model_name == "AttUNet9":
                self.model = UNet(1,1,9 )
        ...
        self.alpha = hyperparams.get('alpha',0.2)
    ...
    def sample_tasks(self):
        ...
        print(datasets_selection)
        print(DATASET_WEIGHTS)
        for ids in datasets_selection:
            name = self.meta_dataset[ids].name
            DATASET_WEIGHTS[name] = (1-self.alpha)*DATASET_WEIGHTS[name]
    ...
    def outer_loop(self, evaluate_every: int = 2, path_to_save="meta_weights.pth"):
        ...
        for meta_epoch in range(self.outer_epochs):
            ...
            wandb.log({"meta_mean_iou": mean_iou, "meta_mean_loss": mean_loss})
    ...
    def setup_model(self):
        if self.model_name == "UNet":
            local_model = UNet_Cells(n_class=1, affine=False, sigmoid=self.sigmoid, norm_type=self.norm_type)
        elif self.model_name == "AttUNet5":
            local_model = UNet(1,1,5 )
        elif self.model_name == "AttUNet2":
            local_model = UNet(1,1,2 )
        elif self.model_name == "AttUNet3":
            local_model = UNet(1,1,3 )
        elif self.model_name == "AttUNet7":
            local_model = UNet(1,1,7 )
        elif self.model_name == "AttUNet9":
            local_model = UNet(1,1,9 )

        # create and setup model
        #local_model = UNet_Cells(n_class=1, affine=False,
        #                         sigmoid=self.sigmoid, norm_type=self.norm_type)
        #local_model = UNet(1,1)
        if torch.cuda.is_available():
            local_model.cuda()
